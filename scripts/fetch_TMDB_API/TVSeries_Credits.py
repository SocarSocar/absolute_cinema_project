#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""
Récupère les crédits d’une série TV via /tv/{series_id}/credits.
Utilise le module fetch_API_TMDB pour la logique commune.
"""

import sys
from pathlib import Path

# Ajouter le dossier parent au path pour importer fetch_API_TMDB
sys.path.insert(0, str(Path(__file__).parent))

from fetch_API_TMDB import TMDBFetcher, select_list_of_dicts


class TVSeriesCreditsFetcher(TMDBFetcher):
    """Fetcher spécifique pour les crédits des séries TV"""
    
    def __init__(self):
        super().__init__(
            input_file="tv_series_dumps.json",                   # fichier source avec les IDs de séries
            output_file="tv_series_credits.ndjson",       # fichier NDJSON de sortie
            log_file="tv_series_credits.log",            # fichier de log
            entity_type="tv series credits",             # type d’entité
            window_days=30,                               # rafraîchir si besoin
            date_field="first_air_date",                  # champ date de référence
            id_field="id",                                # champ identifiant série
            extra_params={}                               # paramètres supplémentaires
        )
    
    def get_endpoint(self, entity_id: int) -> str:
        """Construit l’endpoint pour récupérer les crédits d’une série TV"""
        return f"/tv/{entity_id}/credits"
    
    def project_fields(self, data: dict) -> dict:
        """
        Projection des champs spécifiques aux crédits.
        On ajoute media_type="tv" à chaque entrée
        """
        def add_media_type(entries):
            """Ajoute le champ media_type='tv' à chaque dictionnaire de la liste"""
            return [{**entry, "media_type": "tv"} for entry in entries]

        return {
            "id": data.get("id"),  # ID de la série
            "cast": add_media_type(select_list_of_dicts(
                data.get("cast"),
                ["credit_id", "id", "character", "order"]
            )),
            "crew": add_media_type(select_list_of_dicts(
                data.get("crew"),
                ["credit_id", "id", "department", "job"]
            ))
        }


def main():
    """Point d’entrée principal"""
    fetcher = TVSeriesCreditsFetcher()
    fetcher.run()


if __name__ == "__main__":
    main()

